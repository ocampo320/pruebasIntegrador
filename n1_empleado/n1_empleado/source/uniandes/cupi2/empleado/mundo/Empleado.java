...
public class Empleado
{
    ...
    {
        Fecha hoy = darFechaActual( );
        double antiguedad = fechaIngreso.darDiferenciaEnMeses( hoy ) / 12;
    ...
    public String metodo1( )
    {
    	...
    	if(calcularAntiguedad()==1) {
    		incremento=darSalario()*1.15;
    		 return "El nuevo salario es: $"+incremento;
    	}
    	else
    		if((calcularAntiguedad()>1) &&(calcularAntiguedad()<=2)) {
    			incremento=darSalario()*1.18;
    			 return "El nuevo salario es: $"+incremento;
    		}
    		else
    			if((calcularAntiguedad()>2) &&(calcularAntiguedad()<=3)) {
        			incremento=darSalario()*1.20;
        			 return "El nuevo salario es: $"+incremento;
        		}
    			else {
    			  return "No aplica";
    ...
    public String metodo2( )
    {
    	...
    	if((fechaIngreso.darAnio()<2016) && (calcularEdad()<28)){
    	 prima= (salario * 10)/100;	
    	}
        return "La prima adicional es de:"+prima;
    }
